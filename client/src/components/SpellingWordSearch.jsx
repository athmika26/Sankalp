...
const generateBoard = (size = 9) => {
  ...
  wordsToFind.forEach(word => {
    ...
    // Ensure valid placement of words
    do {
      startRow = Math.floor(Math.random() * size);
      startCol = Math.floor(Math.random() * size);
    } while (
      (direction === 'horizontal' && startCol + word.length > size) ||
      (direction === 'vertical' && startRow + word.length > size)
    );
    ...
    <div className="game-container">
      ...
      {isGameOver && (
        <div className="game-over">
          <p>
            Game Over! You found {score} out of {wordsToFind.length} words!
          </p>
        </div>
      )}
